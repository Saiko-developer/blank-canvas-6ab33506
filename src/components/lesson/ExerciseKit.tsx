...
export function AnswerTryBox({
  ...
  // True / False / Not Mentioned exercises accept both single letters and the
  // full word, case-insensitively. Applied only when the stored answer is a
  // T/F/NM token, so every other exercise type keeps its original matching.
  const TFNM_ACCEPTED: Record<string, string[]> = {
    t: ["t", "true"],
    true: ["t", "true"],
    f: ["f", "false"],
    false: ["f", "false"],
    nm: ["nm", "not mentioned"],
    "not mentioned": ["nm", "not mentioned"],
  };
  ...
  const isMatch = () => {
    ...
    if (TFNM_ACCEPTED[c]) {
      return TFNM_ACCEPTED[c].includes(a);
    }
    return correct
      .split(/[;,/]| or /i)
      .map(normalize)
      .some((cc) => cc === a || (a.length > 3 && cc.includes(a)));
  };
